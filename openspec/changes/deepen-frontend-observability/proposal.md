...
- **Single release version, both runtimes.** Stamp one version identifier at deploy time (`git describe --tags --always`, since the deploy strips `.git` and Composer can't derive the SHA in prod) and feed it to both `OTEL_SERVICE_VERSION` (backend resource) and the Faro `app.version` meta, so errors/traces/events are attributable to a release.
