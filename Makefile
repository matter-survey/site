...
deploy:
	...
	ssh $(SFTP_USER)@$(SFTP_HOST) "cd $(SFTP_PATH) && rm -rf vendor && composer install --no-dev --optimize-autoloader && php bin/console cache:clear --env=prod && php bin/console doctrine:migrations:migrate --no-interaction && php bin/console doctrine:fixtures:load --group=device_types --append --no-interaction"
